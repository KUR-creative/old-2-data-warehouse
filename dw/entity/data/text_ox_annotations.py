...
#---------------------------------------------------------------
def valid(id_tf_seq, has_text_thrshold):
    return True

def load(id_tf_seq, has_text_thrshold):
    ''' 
    id_tf_seq = (crop-id, mask-id|has-text?)s
    
    (crop-id, True) means: Crop have text.
    (crop-id, False) means: Crop have NO text.
    
    (crop-id, mask-id) means: Crop may or may not have text.
    It depends on number of 1 pixels in the mask.
    '''
    # Check crop size = mask size
    # id -> mask crop
